...
def cross_validate(
    ...
    ## UpperMostROWCamera
    #exclude_cameras = ('LowerTrailCamera', 'NorthernTowerMeadowCamera',
    #     'UpperROWWoodChipFieldCamera', 'UpperTrailCamera', 'SODPlotCamera')

    ## NorthernTowerMeadowCamera
    #exclude_cameras = ('UpperMostROWCamera', 'LowerTrailCamera',
    #     'UpperROWWoodChipFieldCamera', 'UpperTrailCamera', 'SODPlotCamera')

    ## UperROWWoodChipFieldCamera
    #exclude_cameras = ('UpperMostROWCamera', 'NorthernTowerMeadowCamera',
    #      'LowerTrailCamera', 'UpperTrailCamera', 'SODPlotCamera')

    ## UpperTrailCamera
    #exclude_cameras = ('UpperMostROWCamera', 'NorthernTowerMeadowCamera',
    #       'UpperROWWoodChipFieldCamera', 'LowerTrailCamera', 'SODPlotCamera')

    ## SODPlotCamera
    #exclude_cameras = ('UpperMostROWCamera', 'NorthernTowerMeadowCamera',
    #       'UpperROWWoodChipFieldCamera', 'UpperTrailCamera', 'LowerTrailCamera')

    if exclude_cameras is not None:
    ...
    #"""
    #Bellow Removes classes that contain images under a choosen ammount
    #this helps with improving accuracy when comparing results across camera groups
    cut_value = 200
    low_groups = []
    tmp = 0
    for i in range (0,np.size(classes)):
        current = classes[i]
        count = 0
        while(current == class_labels[tmp]):
            count = count + 1
            if(tmp + 1 < np.size(file_names)):
                tmp = tmp + 1
            else:
                current = "Done"
        print(classes[i], " ", count)
        if(count < cut_value):
            low_groups.append(classes[i])

    #"""
    if low_groups is not []:
        print("Removing", low_groups, "classes less than ", cut_value)
        excluded = exclude_classes(low_groups, class_labels)
        class_labels = np.delete(class_labels, excluded, 0)
        file_names = np.delete(file_names, excluded, 0)
        group_labels = np.delete(group_labels, excluded, 0)
        features = np.delete(features, excluded, 0)
    ...
    if summarize_misclassified_images is not None:
        ...
        misclassified_indexes = np.random.choice(misclassified.reshape(-1), 25, replace=True)
        ...
        csv = tmp_dir + 'results/classified_images.csv'
        with open(csv, 'w', newline="") as f:
            f.write('file_name,predicted_class,score\n')
            for m in classified.reshape(-1):
                row = file_names_test[m] + ',' + classes[predicted_classes[m]] + ',' + str(prediction_scores[m])
                f.write("%s\n" % row)
        print("csv of all classified images saved to", csv)
